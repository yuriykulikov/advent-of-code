...
class Day8IHeardYouLikeRegisters {
    ...
    class OP(
        ...
        val signedArg = if (inc) arg else -arg
    }

    fun parse(input: String): List<OP> {
        return input.lines()
            .map { line ->
    ...
    @Test
    fun test() {
        assertThat(parse(testInput).foldOperations().values.max()).isEqualTo(1)
    }
    ...
    fun List<OP>.foldOperations(): PersistentMap<String, Int> {
        return scanOperations().last()
    }

    fun List<OP>.scanOperations(): List<PersistentMap<String, Int>> {
        return asSequence().scan(persistentMapOf<String, Int>()) { regs, op ->
            val condRegVal = regs.getOrDefault(op.condReg, 0)
            val exec = when (op.condCond) {
                ">=" -> condRegVal >= op.condVal
                "<=" -> condRegVal <= op.condVal
                ">" -> condRegVal > op.condVal
                "<" -> condRegVal < op.condVal
                "==" -> condRegVal == op.condVal
                "!=" -> condRegVal != op.condVal
                else -> throw Exception("Unsupported ${op.condCond}")
            }

            if (exec) {
                regs.put(op.reg, regs.getOrDefault(op.reg, 0) + op.signedArg)
            } else {
                regs
            }
        }.toList()
