...
class Day7RecursiveCircus {

    @Test
    fun star1Test() {
        val nodes = parseInput(testInput)
        assertThat(findRoot(nodes).name).isEqualTo("tknk")
    }

    @Test
    fun star1() {
        val nodes = parseInput(taskInput)
        assertThat(findRoot(nodes).name).isEqualTo("gynfwly")
    }
    ...
    fun star2() {
        val nodes = parseInput(taskInput)
        val nodesByName: Map<String, Node> = nodes.groupBy { it.name }.mapValues { (k, v) -> v.first() }
        fun String.node(): Node = nodesByName.getValue(this)

        fun Node.weight(): Int = ownWeight + children.map { child -> child.node().weight() }.sum()
        val root = findRoot(nodes)

        fun Node.correctedNodeWeight(): Int? {
            println("calc supposedWeight of $this")
            require(children.isNotEmpty()) { "Node $this has no children!" }

            val weights = children.map { it.node().weight() }

            return if (weights.distinct().size == 1) {
                println("Children of node $this are balanced")
                null
            } else {

                val unbalancedWeight = weights.first { weight -> weights.count { it == weight } == 1 }

                // val unbalancedWeight = weights.groupBy { it }.values.first { it.size == 1 }.first()
                val unbalanced = children.first { it.node().weight() == unbalancedWeight }.node()

                unbalanced.correctedNodeWeight() ?: run {
                    // children of unbalanced node are balanced, this means this node has wrong weight. It has to have another weight!
                    val balancedWeight = children.first { it != unbalanced.name }.node().weight()
                    val unbalancedChildrenWeight = unbalanced.children.map { it.node().weight() }.sum()
                    balancedWeight - unbalancedChildrenWeight
                }
            }
        }

        assertThat(root.correctedNodeWeight()).isEqualTo(1526)
    }

    private fun findRoot(nodes: List<Node>): Node {
        val candidates = nodes
            .filter { it.children.isNotEmpty() }

        val allChildren = candidates.flatMap { node -> node.children }
            .distinct()

        return candidates.first { node ->
            !allChildren.contains(node.name)
        }
    }

    private fun parseInput(input: String): List<Node> {
        return input
            .lines()
            .map { line ->
                val split = line
                ...
                Node(split[0], split[1].toInt(), split.subList(2, split.size))
            }
    }
