...
class Day4Passphrase {
    @Test
    fun `there are 455 valid pass phrases`() {
        val validPhrases = input.lines()
        ...
        assertThat(validPhrases).hasSize(455)
    }

    @Test
    fun `there are 455 valid pass phrases for the second star`() {
        val validPhrases = input.lines()
            .filterNot { it.isEmpty() }
            .filter { line ->
                val split = line.split(" ")
                split.size == split.distinctBy {
                    it.toSortedSet()
                }.size
            }

        assertThat(validPhrases).hasSize(186)
    }
