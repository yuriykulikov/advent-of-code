...
fn parse(input: &str) -> (Vec<u32>, Vec<Board>) {
    ...
    let seq = seq_str.split(",").map(|it| u32::from_str(it).unwrap()).collect();

    let boards = all_boards_str.split("\n\n")
        .map(|board_str| {
            let rows: Vec<Vec<u32>> = board_str.lines().into_iter()
                .filter(|it| !it.is_empty())
                .map(|line| {
                    line.split(char::is_whitespace).filter_map(|it| u32::from_str(it).ok()).collect()
                }).collect();
    ...
    let (winning_board, subsequence) = (0..seq.len())
        ...
        .filter_map(|subsequence| {
            boards.iter().find(|board| {
                ...
                let has_crossed_column = board.columns.iter().any(|column| column.iter().all(|e| subsequence.contains(e)));
                let has_crossed_row = board.rows.iter().any(|row| row.iter().all(|e| subsequence.contains(e)));
...
fn find_last_winning_board<'a>(seq: &'a Vec<u32>, boards: &'a Vec<Board>) -> (&'a Board, &'a [u32]) {
    ...
    let last_board_to_win = boards.iter().find(|board| {
        // find the board which does not win with this sequence
        let has_crossed_column = board.columns.iter().any(|column| column.iter().all(|e| prev_subsequence.contains(e)));
        let has_crossed_row = board.rows.iter().any(|row| row.iter().all(|e| prev_subsequence.contains(e)));
        !has_crossed_column && !has_crossed_row
    })
        .unwrap();
...
fn calc_score(winning_board: &Board, subsequence: &[u32]) -> u32 {
    winning_board.rows.iter().flatten()
...
mod test {
    ...
    fn parsing_test() {
        let (seq, boards) = parse(include_str!("day_04_test_input"));
        assert_eq!(seq, vec![7, 4, 9, 5, 11, 17, 23, 2, 0, 14, 21, 24, 10, 16, 13, 6, 15, 25, 12, 22, 18, 20, 8, 19, 3, 26, 1]);
        assert_eq!(boards.first().unwrap().rows, vec![
            vec![22, 13, 17, 11, 0],
            vec![8, 2, 23, 4, 24],
            vec![21, 9, 14, 16, 7],
            vec![6, 10, 3, 18, 5],
        ...
        assert_eq!(boards.first().unwrap().columns, vec![
            vec![22,
                 8,
                 21,
                 6,
