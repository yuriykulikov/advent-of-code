 ...
 *
 * I have tried to code something which finds common chunks and marks them, but in the end I did it in Notepad++
 */
class Day17SetAndForget {

    private fun parseMap(mapString: String): Map<Point, Char> {
        return mapString
            .lines()
            .mapIndexed { y, line ->
                line.mapIndexed { x, c ->
    ...
    fun calculateAlignments(map: Map<Point, Char>): Int {
        val intersections = map.keys
            .filter { point ->
                map[point] == '#'
                        && map[point.left()] == '#'
                        && map[point.up()] == '#'
        ...
        val withIntersections = map.toPersistentMap().putAll(intersections)

        printMap(withIntersections, invertedY = true) { "$it" }

        return withIntersections.entries
...
..#####...^..
""".trimIndent()
        assertThat(calculateAlignments(parseMap(map))).isEqualTo(76)
    }

    @Test
    fun silver() {
        val initialMap = intCodeComputer(ascii.split(",").map { it.toInt() }.toPersistentList())
            .runToHalt()
            .out
            .map { it.toChar() }
            .joinToString("")

        val map: Map<Point, Char> = parseMap(initialMap)

        assertThat(calculateAlignments(map)).isEqualTo(7816)
    }
    ...
    @Test
    fun gold() {
        val frameSize = intCodeComputer(program = ascii, mutable = false)
            .runToHalt()
            .out
            .size
