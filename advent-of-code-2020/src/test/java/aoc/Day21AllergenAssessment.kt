...
class Day21AllergenAssessment {
    ...
    0000000000000000000000000000000000000000 * In the above example, none of the ingredients kfcds, nhms, sbzzf, or trh can contain an allergen.
    ...
    @Test
    fun silverTest() {
        assertThat(countAllergenFreeIngredients(testInput)).isEqualTo(5)
    }

    @Test
    fun silver() {
        assertThat(countAllergenFreeIngredients(taskInput)).isEqualTo(1882)
    }

    @Test
    fun goldTest() {
        assertThat(canonicalDangerousIngredientList(testInput)).isEqualTo("mxmxvkd,sqjhc,fvjkl")
    }

    @Test
    fun gold() {
        assertThat(canonicalDangerousIngredientList(taskInput)).isEqualTo("xgtj,ztdctgq,bdnrnx,cdvjp,jdggtft,mdbq,rmd,lgllb")
    }

    private fun countAllergenFreeIngredients(input: String): Int {
        val ingredientsAndAllergens: List<Pair<List<Ingredient>, List<Allergen>>> = parse(input)

        val allergenToIngredient = findMatches(ingredientsAndAllergens)

        val allergic = allergenToIngredient.values.toSet()
        return ingredientsAndAllergens.flatMap { it.first }
            .count { it !in allergic }
    }

    private fun canonicalDangerousIngredientList(input: String): String {
        val ingredientsAndAllergens: List<Pair<List<Ingredient>, List<Allergen>>> = parse(input)
        return findMatches(ingredientsAndAllergens)
            .entries
            .sortedBy { (allergen, ingredient) -> allergen.value }
            .joinToString(",") { (allergen, ingredient) -> ingredient.value }
    }

    private fun findMatches(ingredientsAndAllergens: List<Pair<List<Ingredient>, List<Allergen>>>) =
        ingredientsAndAllergens
            ...
            .groupBy(
                keySelector = { (allergen, ingredients) -> allergen },
                valueTransform = { (allergen: Allergen, ingredients: List<Ingredient>) -> ingredients.toSet() }
            )
            .mapValues { (allergen, ingredientsCandidates) ->
                ingredientsCandidates.reduce { l, r -> l.intersect(r) }
            }
            .entries
            .sortedBy { (allergen, possibleIngredients) -> possibleIngredients.size }
            .onEach { println(it) }
            .fold(setOf()) { confirmed: Set<Pair<Allergen, Ingredient>>, (allergen, ingredients) ->
                val possibleIngredients = ingredients.minus(confirmed.map { it.second })
                possibleIngredients.firstOrNull()
                    // found match
                    ?.let { confirmed.plus(allergen to it) }
                    ?: confirmed
            }.toMap()

    private fun parse(input: String) = input.lines().map { line ->
        require(line.contains("(contains"))
        val ingredients: List<Ingredient> = line.substringBefore(" (").split(" ").map { Ingredient(it) }
        val allergens: List<Allergen> =
            line.substringAfter("(contains ").removeSuffix(")").split(", ").map { Allergen(it) }
