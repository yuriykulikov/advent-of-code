...
class Day2PasswordPhilosophy {
    @Test
    fun testSilver() {
        assertThat(countValidRentalPasswords(inputTest)).isEqualTo(2)
    }

    @Test
    fun silver() {
        assertThat(countValidRentalPasswords(input)).isEqualTo(378)
    }

    @Test
    fun testGold() {
        assertThat(countValidTobogganPasswords(inputTest)).isEqualTo(1)
    }

    @Test
    fun gold() {
        assertThat(countValidTobogganPasswords(input)).isEqualTo(280)
    }
    ...
    data class Password(
        ...
        companion object {
            // 1-3 a: abcde
            fun fromString(string: String): Password {
                return with(string) {
                    Password(
                        letter = substringAfter(" ").substringBefore(":").first(),
                        first = substringBefore("-").toInt(),
                        second = substringAfter("-").substringBefore(" ").toInt(),
                        str = substringAfter(": ")
                    )
                }
            }
    ...
    private fun countValidRentalPasswords(input: String): Int {
        return input.lines()
            .filter { it.isNotBlank() }
            .map { line -> Password.fromString(line.trim()) }
            .count { password ->
                val count = password.str.count { it == password.letter }
                count >= password.first && count <= password.second
            }
    }

    private fun countValidTobogganPasswords(input: String): Int {
        return input.lines()
            .filter { it.isNotBlank() }
            .map { line -> Password.fromString(line.trim()) }
            .count { password ->
                val first = password.str[password.first - 1]
                val second = password.str[password.second - 1]
                val differ = first != second
                differ && (first == password.letter || second == password.letter)
            }
    }
