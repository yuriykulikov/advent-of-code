...
class Day4PassportProcessing {
    companion object {
        val mandatoryFieldNames = listOf("byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid")
    }

    data class Passport(val fields: Map<String, String>)

    private fun Passport.allFieldsPresent(): Boolean {
        return fields.keys.containsAll(mandatoryFieldNames)
    }

    private fun parsePassports(str: String): List<Passport> {
        return str.split("\n\n")
            .map { passString -> parsePassport(passString) }
    }

    private fun parsePassport(passString: String): Passport {
        val fields = passString.split(" ", "\n")
            .map { it.trim() }
    ...
    @Test
    fun silverTest() {
        val validCount = parsePassports(testInput)
            .filter { it.allFieldsPresent() }
            .count()
    ...
    @Test
    fun silver() {
        val validCount = parsePassports(input)
            .filter { it.allFieldsPresent() }
            .count()

        assertThat(validCount).isEqualTo(210)
    }
    ...
    private fun Passport.isValid(): Boolean {
        ...
        return listOf(
            fields.getValue("byr").run {
                length == 4 && toInt() >= 1920 && toInt() <= 2002
            },
            fields.getValue("iyr").run {
                length == 4 && toInt() >= 2010 && toInt() <= 2020
            },
            fields.getValue("eyr").run {
                length == 4 && toInt() >= 2020 && toInt() <= 2030
            },
            fields.getValue("hgt").run {
                when {
            ...
            fields.getValue("hcl").run {
                startsWith("#") && length == 7 && substringAfter("#").toIntOrNull(16) != null
            },
            fields.getValue("ecl").run {
                this in listOf("amb", "blu", "brn", "gry", "grn", "hzl", "oth")
            },
            fields.getValue("pid").run {
                length == 9 && toIntOrNull() != null
            }
        ).all { it }
    }
    ...
    fun gold() {
        ...
        assertThat(validCount).isEqualTo(131)
    }
