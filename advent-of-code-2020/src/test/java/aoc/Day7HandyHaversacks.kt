...
class Day7HandyHaversacks {
    ...
    @Test
    fun silver() {
        val parsed = parseBagPolicy(input)
        assertThat(countAllPossibleParentsOfShinyGoldBag(parsed)).isEqualTo(348)
    }

    private fun countAllPossibleParentsOfShinyGoldBag(
        policies: List<BagPolicy>,
    ): Int {
        val childToParent = mapChildrenToParents(policies)

        fun withAllParents(childName: String): Set<String> {
            val parents = childToParent.getOrDefault(childName, emptySet())
                .flatMap { withAllParents(it) }
                .toSet()
            return parents.plus(childName)
        }

        return withAllParents("shiny gold").minus("shiny gold").count()
    }

    /**
     * Builds a map containing possible parents of each child.
     */
    private fun mapChildrenToParents(policies: List<BagPolicy>): Map<String, Set<String>> {
        return mutableMapOf<String, MutableSet<String>>().apply {
            policies.forEach { bagPolicy ->
                bagPolicy.children.forEach { innerBag ->
                    getOrPut(innerBag.name) { mutableSetOf() }
                        .add(bagPolicy.name)
                }
            }
        }.toMap()
    ...
    fun goldTest() {
        val parsed = parseBagPolicy(testInput)
        assertThat(countChildrenOfShinyBag(parsed)).isEqualTo(32)
    }

    @Test
    fun goldTest2() {
        val parsed = parseBagPolicy(testInput2)
        assertThat(countChildrenOfShinyBag(parsed)).isEqualTo(126)
    }

    @Test
    fun gold() {
        val parsed = parseBagPolicy(input)
        assertThat(countChildrenOfShinyBag(parsed)).isEqualTo(18885)
    }

    data class BagPolicy(
        val name: String,
        val children: List<InnerBag>,
    )
    ...
    /** Counts all children of a shiny gold bag recursively */
    private fun countChildrenOfShinyBag(policies: List<BagPolicy>): Int {
        val bagToContents: Map<String, List<InnerBag>> = policies.associateBy(
            keySelector = { it.name },
            valueTransform = { it.children }
        )

        fun countAllChildrenOf(parent: String): Int {
            val innerBags = bagToContents.getValue(parent)
            return innerBags.sumBy { (name, amount) ->
                // bag itself, plus all its children times amount of bags
                (1 + countAllChildrenOf(name)) * amount
            }
        }

        return countAllChildrenOf("shiny gold")
    }

    private fun parseBagPolicy(input: String): List<BagPolicy> {
        return input.lines()
            .map {
                it
                    .replace(" bags", "")
            ...
            .map { line ->
                val name = line.substringBefore(" contain ")
                val children: List<InnerBag> =
                    when {
                        line.endsWith("no other") -> emptyList()
                        else -> line.substringAfter(" contain ")
                            .split(", ")
                            .map { child ->
                                InnerBag(
                                    child.substringAfter(" "),
                                    child.substringBefore(" ").toInt(),
                                )
                            }
                    }
                BagPolicy(name, children)
            }
    }
